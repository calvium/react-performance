...
import {getReactInternal} from '../utils';
...
const measure: MeasureType = ({ getId, Component, isCollapsed = true }) => {
  ...
  return React.forwardRef((props, ref) => {
    return <Measured {...props} forwardedRef={ref} />;
  });
}
...
const createRecordItem = ({ element, componentName }) => {
  // $FlowFixMe
  const owner = getReactInternal(element)._debugOwner;
  const ownerName = (owner !== null && typeof owner !== 'undefined') ? getReactInternal(element)._debugOwner.type.name : "";
